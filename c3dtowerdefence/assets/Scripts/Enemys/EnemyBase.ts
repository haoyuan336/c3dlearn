import { _decorator, Component, Node, CCInteger, v3, Vec3, tween, path, Tween, CameraComponent, Vec2, v2, JsonAsset, game, isValid, ProgressBarComponent } from 'cc';
...
export class EnemyBase extends BaseObject {
    ...
    public init(gameConfig: JsonAsset, pathList: Node[] ){
    ...
    onLoad() {
        ...
        this.state.addState("run", () => {
            ...
            for (let i = 1; i < this.pathList.length; i++) {
                ...
                tw.to(moveTime, { position: v3(nextPos.x, 0.5, nextPos.z) });
            }
            tw.call(()=>{
